const search = () => {
    ...
    <div role="status" class="flex flex-col items-center justify-center">
        ...
        <span class="">Crawling the web for product...</span>
    </div>
</div>`
	...
	fetchData().then((data) => {
		document
			...
			.insertAdjacentHTML('beforebegin', '<h1>Search Results</h1>')
		data.map((item) => {
			const markup = `
<div class="w-full bg-white rounded-lg shadow-md hover:shadow-lg dark:bg-gray-800 dark:border-gray-700">
